...
> **Note:** all prices in this guide use the game's currency, **Tesserae**, written as `t` (e.g. a turnip seed costs 25t and sells for 40t). Some older pages on this site still show outdated prices under a `g` symbol — those legacy pages are being cleaned up, so trust this guide's figures.
